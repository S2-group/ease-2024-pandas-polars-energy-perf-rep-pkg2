...
def FileTypes(output_files, data_file):
        ...
        df.head(375000).write_json(PolarsBig_json)  # Converts each row to a JSON object

        df.head(375000).write_parquet(PolarsBig_parquet)  # Converts each row to a JSON object
